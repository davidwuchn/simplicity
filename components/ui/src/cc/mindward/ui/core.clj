          ...
          (h/html
           [:html
            ...
            [:body
             ...
             ;; Canvas (Full Screen)
             [:canvas {:id "gameCanvas" :class "cursor-none"}]
             ;; HUD Overlay (High Score)
             [:div {:class "absolute top-4 right-4 pointer-events-none z-10"}
              [:div {:class "text-xl font-mono text-cyan-400 bg-black/50 p-2 border border-cyan-400"}
               "BEST: " [:span {:id "high-score" :class "text-white"} high-score]]]
             ;; Controls Overlay
             [:div {:class "absolute bottom-4 right-4 text-gray-500 text-xs font-mono pointer-events-none text-right bg-black/50 p-2"}
              [:div "ARROWS to Move"]
